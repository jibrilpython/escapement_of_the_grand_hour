...
class _StatsScreenState extends ConsumerState<StatsScreen>
  ...
  RateDetectionClass? _filterRate;
  ...
  List<SafetyMechanismModel> _filtered(List<SafetyMechanismModel> all) {
    if (_filterRate == null) return all;
    return all.where((e) => e.rateDetectionClass == _filterRate).toList();
  }
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      ...
                      setState(() => _filterRate = null);
                    },
                    child: AnimatedOpacity(
                      opacity: _filterRate != null ? 1.0 : 0.0,
  ...
  Widget _buildFilterRow(List<SafetyMechanismModel> all) {
    final rateClasses = all.map((e) => e.rateDetectionClass).toSet().toList()
      ..sort((a, b) => a.label.compareTo(b.label));
    if (rateClasses.length < 2) return const SizedBox.shrink();

    return SizedBox(
      height: 32.h,
      child: ListView.separated(
        scrollDirection: Axis.horizontal,
        itemCount: rateClasses.length,
        separatorBuilder: (context, index) => SizedBox(width: 6.w),
        itemBuilder: (context, i) {
          final gc = rateClasses[i];
          final active = _filterRate == gc;
          final color = getRateDetectionColor(gc);
          return GestureDetector(
            onTap: () {
              HapticFeedback.selectionClick();
              setState(() => _filterRate = active ? null : gc);
            },
  ...
  Widget _buildRingAndStats(List<SafetyMechanismModel> entries, int totalAll) {
    ...
    final ringLabels = ['Cabinet health', 'Total count', 'At risk'];
    ...
    final statItems = [
      _StatItem(total.toString(), 'MECHANISMS', kAccent),
  ...
  String _statDetail(String label, int totalAll, List<SafetyMechanismModel> filtered) {
    switch (label) {
      case 'MECHANISMS':
        ...
        return filteredOut > 0
            ...
            : '$totalAll specimens archived in this cabinet.';
      case 'HEALTHY':
      ...
      case 'SCORE':
        return 'Tap the ring to cycle between cabinet health, total count, and at-risk specimens.';
      case 'SPAN':
  ...
  // ── Era timeline ─────────────────────────────────────────────────────────────
  Widget _buildEraTimeline(List<SafetyMechanismModel> entries) {
      ...
      children: [
        ...
        if (_selectedEra != null)
          Padding(
            padding: EdgeInsets.only(top: 8.h),
            child: Text(
              '${yearCounts[_selectedEra]} mechanism${yearCounts[_selectedEra]! > 1 ? 's' : ''} from $_selectedEra.',
              style: GoogleFonts.sourceSans3(color: kSecondaryText, fontSize: 11.sp),
            ),
          ),
      ],
    );
  ...
  Widget _buildInventoryGrid(List<SafetyMechanismModel> entries) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSectionHeader('PALLET MATERIAL INVENTORY'),
        SizedBox(height: 16.h),
        Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Expanded(
              child: _buildInventoryColumn('PALLET MATERIAL', bodySorted, (e) => getBodyMetalColor(e.key)),
            ),
            SizedBox(width: 12.w),
            Expanded(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        ...
                        style: GoogleFonts.sourceSans3(color: kPrimaryText, fontSize: 10.sp),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
                    ),
  ...
  // ── Condition breakdown ──────────────────────────────────────────────────────
  Widget _buildConditionBreakdown(List<SafetyMechanismModel> entries) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSectionHeader('SOUNDNESS BREAKDOWN'),
        SizedBox(height: 16.h),
        ClipRRect(
          borderRadius: BorderRadius.circular(kRadiusPill),
          child: SizedBox(
                ...
                return Expanded(
                  ...
                  child: GestureDetector(
                    onTap: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(s.count > 1 ? '${s.label}: ${s.count} mechanisms (${(s.fraction * 100).round()}%)' : '${s.label}: ${s.count} mechanism (${(s.fraction * 100).round()}%)'),
                          duration: const Duration(milliseconds: 1200),
            ...
            return GestureDetector(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    '${s.label} ${s.count}',
                    style: GoogleFonts.sourceSans3(color: kSecondaryText, fontSize: 9.sp),
                  ),
                ],
              ),
            );
  ...
  String _getEraSpan(List<SafetyMechanismModel> entries) {
    final years = <int>[];
    final regex = RegExp(r'\d{4}');
    for (final e in entries) {
      for (final m in regex.allMatches(e.eraOfProduction)) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 16.h),
          Text(
            _filterRate != null ? 'NO MATCHING MECHANISMS' : 'AWAITING MECHANISM DATA',
            style: GoogleFonts.ibmPlexMono(color: kSecondaryText, fontSize: 12.sp),
          ),
          if (_filterRate != null) ...[
            SizedBox(height: 8.h),
            GestureDetector(
              onTap: () => setState(() => _filterRate = null),
              child: Text(
                'CLEAR FILTER',
